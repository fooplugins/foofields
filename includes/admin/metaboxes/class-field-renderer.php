...
//					?><!--</div>--><?php
		...
		static function render_field( $field, $field_attributes = array() ) {
			...
			switch ( $type ) {
				...
				case 'selectize-multi':
					self::render_selectize_multi_field( $field );
		...
		/**
		 * Render the HTML needed for a selectize multi control
		 *
		 * @param $field
		 */
		static function render_selectize_multi_field( $field ) {
			global $post;
			$inner = '';
